...
// Provjeri je li blok size×size slobodan ILI prekriven cestom (za vozila)
boolean jeslobodnoZaVozilo(int gx, int gz, int size) {
  for (int dx = 0; dx < size; dx++) {
    for (int dz = 0; dz < size; dz++) {
      // Ako polje nije zauzeto, ok
      if (!zauzetiGridovi.contains(gridKey(gx + dx, gz + dz))) continue;
      // Polje je zauzeto — provjeri je li to cesta
      if (!jeCestaNaPolju(gx + dx, gz + dz)) return false;
    }
  }
  return true;
}

// Vraća true ako na danom polju postoji postavljeni objekt kategorije "Ceste" (1)
boolean jeCestaNaPolju(int gx, int gz) {
  for (PlacedObject obj : placedObjects) {
    InventoryItem item = allItems.get(obj.typeIndex);
    if (item.kategorija != 1) continue; // 1 = Ceste
    if (gx >= obj.gridX && gx < obj.gridX + item.gridVelicina &&
        gz >= obj.gridZ && gz < obj.gridZ + item.gridVelicina) {
      return true;
    }
  }
  return false;
...
void drawPlacementPreview() {
  ...
  boolean slobodno = (item.kategorija == 0)
                   ? jeslobodnoZaVozilo(lastGridX, lastGridZ, item.gridVelicina)
                   : jeslobodno(lastGridX, lastGridZ, item.gridVelicina);
  noStroke();
